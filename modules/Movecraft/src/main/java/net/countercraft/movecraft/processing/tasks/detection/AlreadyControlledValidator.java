...
public class AlreadyControlledValidator implements TaskPredicate<MovecraftLocation> {
    ...
    public Result validate(@NotNull MovecraftLocation location, @NotNull CraftType type, @NotNull MovecraftWorld world, @Nullable CommandSender player) {
        return CraftManager.getInstance().getCraftList().stream()
                .filter((c)->c.getMovecraftWorld().equals(world))
                .filter(c -> c.getType().getCraftName().equals(type.getCraftName()))
                .map(Craft::getHitBox)
                .anyMatch((h) -> h.contains(location)) ? Result.failWithMessage(I18nSupport.getInternationalisedString("Detection - Failed Craft is already being controlled")) : Result.succeed();
    }
}

...
public class TranslationTask extends AsyncTask {
    ...
    private final Set<Material> passthroughBlocks = craft.getType().getMaterialSetProperty(CraftType.PASSTHROUGH_BLOCKS);
    ...
    protected void execute() throws InterruptedException, ExecutionException {
        ...
        if (world.equals(craft.getWorld()) && craft.getType().getBoolProperty(CraftType.USE_GRAVITY) && !craft.getSinking()) {
            ...
            if (incline > 0){
                ...
                while (w.getBlockAt(midPoint.getX(), groundY - 1, midPoint.getZ()).getType().isAir() || passthroughBlocks.contains(w.getBlockAt(midPoint.getX(), groundY - 1, midPoint.getZ()).getType())){
                    groundY--;
                }
        ...
        for(MovecraftLocation oldLocation : oldHitBox){
            ...
            if (craft.getSinking()) {
                ...
                blockObstructed = !testMaterial.isAir() && !passthroughBlocks.contains(testMaterial);
            }

            boolean ignoreBlock = world.getBlockAt(oldLocation.getX(), oldLocation.getY(), oldLocation.getZ()).getType().isAir() && blockObstructed;
            ...
            if (blockObstructed && !harvestBlocks.isEmpty() && harvestBlocks.contains(testMaterial)) {
                Material tmpType = world.getType(oldLocation.getX(), oldLocation.getY(), oldLocation.getZ());
        ...
        if (craft.getType().getMaterialSetProperty(CraftType.FORBIDDEN_HOVER_OVER_BLOCKS).size() > 0){
            ...
            while (world.getBlockAt(test.getX(), test.getY(), test.getZ()).getType().isAir()){
                test = test.translate(0, -1, 0);
            }
            Material testType = world.getBlockAt(test.getX(), test.getY(), test.getZ()).getType();
        ...
        if(craft.getSinking()){
            ...
            for(MovecraftLocation location: oldHitBox){
                if(world.getBlockAt(location.getX(), location.getY(), location.getZ()).getType().isAir()){
    ...
    private MovecraftLocation surfaceLoc(MovecraftLocation ml) {
        ...
        do {
            ...
            testType = world.getBlockAt(surfaceLoc.getX(), surfaceLoc.getY(), surfaceLoc.getZ()).getType();
        } while ((!testType.isAir() &&
                !passthroughBlocks.contains(testType) &&
                !oldHitBox.contains(surfaceLoc)) &&
                surfaceLoc.getY() + 1 > maxHeightLimit);
        return surfaceLoc;
    }
    ...
    private int inclineCraft(HitBox hitBox){
        ...
        var harvesterBladeBlocks = craft.getType().getMaterialSetProperty(CraftType.HARVESTER_BLADE_BLOCKS);
        ...
        for (MovecraftLocation ml : collisionBox){
            ...
            if (testType.isAir() || passthroughBlocks.contains(testType) ||
                    harvestBlocks.contains(testType) && harvesterBladeBlocks.contains(world.getType(below.getX(), below.getY(), below.getZ()))) {
                continue;
            }
    ...
    private int dropDistance (HitBox hitBox) {
        ...
        do {
            ...
            for (MovecraftLocation ml : bottomLocs) {
                ...
                Material testType = world.getBlockAt(dropped.getX(), dropped.getY(), dropped.getZ()).getType();
    ...
    private boolean isOnGround(HitBox hitBox){
        ...
        for (MovecraftLocation bottomLoc : bottomLocs){
            ...
            var belowBottomLoc = bottomLoc.translate(0, -1, 0);
            Material testType = world.getBlockAt(belowBottomLoc.getX(), belowBottomLoc.getY(), belowBottomLoc.getZ()).getType();
            //If the lowest part of the bottom locs touch the ground, return true anyways
            if (testType.isAir())
                continue;
            else if (passthroughBlocks.contains(testType))
                continue;
        ...
        for (MovecraftLocation translatedBottomLoc : translatedBottomLocs){
            ...
            Material testType = world.getBlockAt(beneath.getX(), beneath.getY(), beneath.getZ()).getType();
